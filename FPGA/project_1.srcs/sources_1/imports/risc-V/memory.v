...
module ROM(Addr, Dout);
    ...
    initial begin
        $readmemh("C:\\Users\\Miles\\Desktop\\Computer-Organization-Big-Homework\\FPGA\\risc-v-benchmark.txt", rom);    // Read initialization data from a file to fill the read-only memory
    end

    always @(Addr) begin
        Dout <= rom[Addr];      // Output the corresponding data based on the input address
    end
endmodule
